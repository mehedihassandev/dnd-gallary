...
const App = () => {
  ...
  const handleDelete = () => {
    const remainingItems = itemData.filter(
      (item) => !selectedItems.includes(item.img)
    );
    itemData.length = 0; // Clear the array
    itemData.push(...remainingItems); // Add back the remaining items
              ...
              {selectedItems.length > 0 ? (
                <>
                  <Typography variant="body1" pr={2} fontWeight="bold">
                    <Checkbox
                      checked={selectedItems.length === itemData.length}
                      onChange={() => {
                        if (selectedItems.length === itemData.length) {
                          setSelectedItems([]);
                        } else {
                          setSelectedItems(itemData.map((item) => item.img));
                        }
                      }}
                    />{" "}
